...
def profile_view(request):
    ...
    # Structured data for the Overview tab
    profile_fields = [
    ...
    # Wallet addresses for the Wallets tab
    # Each entry: (bootstrap-icon class, label, saved value, chain key for copy button id)
    wallet_fields = [
        ("bi bi-currency-bitcoin text-warning", "Bitcoin (BTC)",       profile.bitcoin_id,    "btc"),
        ("bi bi-currency-exchange text-primary","Ethereum (ETH/ERC20)", profile.ethereum_id,   "eth"),
        ("bi bi-circle-fill text-danger",        "USDT TRC20",          profile.usdt_trc20_id, "usdt"),
        ("bi bi-circle-fill text-danger",        "Tron (TRX)",          profile.tron_id,       "trx"),
        ("bi bi-currency-exchange text-warning", "BEP20 (Binance)",     profile.bep20_id,      "bep20"),
    ]
...
def edit_profile_view(request):
    ...
    if request.method == "POST":
        ...
        # Pass instance so ModelForm pre-fills ALL fields including wallet addresses
